    ...
    async def _status_consumer(self):
        ...
        async def worker():
            async for env in self.bus.subscribe("worker/done"):
                ...
                await self.bus.publish("ui/print", {"text": f"{self.name}: ✅ Done: {summary}"}, sender=self.name, job_id=env.job_id)

        async def tests():
            async for env in self.bus.subscribe_prefix("test/"):
                if env.topic.endswith("passed"):
                    await self.bus.publish("ui/print", {"text": f"{self.name}: ✅ Tests PASS."}, sender=self.name, job_id=env.job_id)
                elif env.topic.endswith("failed"):
                    reason = (env.payload or {}).get("reason") or "Unknown"
                    # IMPORTANT: do NOT auto-refine; just report failure
                    await self.bus.publish("ui/print", {"text": f"{self.name}: ❌ Tests FAIL — {reason}"}, sender=self.name, job_id=env.job_id)
