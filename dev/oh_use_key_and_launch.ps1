...
# --- API key (as requested) ---
$KEY='REDACTED'
Set-Content -Encoding Ascii -LiteralPath 'api_key.txt' -Value $KEY
try { setx OPENAI_API_KEY $KEY | Out-Null } catch {}
...
# --- dev/brain_chat_shell.py (tool-calling + planner + tail poll) ---
$pycode=@'
...
def _chat_turn(messages):
    from openai import OpenAI
    key=_get_key()
    if not key: return {"final": "(no OPENAI_API_KEY/api_key.txt)", "messages": messages}
    client=OpenAI(api_key=key)
    r=client.chat.completions.create(model=MODEL_NAME, messages=messages, tools=TOOL_SPEC, tool_choice="auto")
...
def main():
    ...
    messages=[{"role":"system","content":"You are the Ecosystem Brain on Windows. Act autonomously. If a local tool helps, call it. Be concise."}]
    while True:
        ...
        ans=turn.get('final','(no answer)'); append('assistant', ans); print(ans)
        extra=poll_tail(timeout=10, min_wait=0.8)
        if extra and extra.strip() and extra.strip()!=ans.strip(): print(f'[ecosystem] {extra}')
    print('Bye.')
...
'@
Set-Content -Encoding Ascii -LiteralPath 'dev\_smoke.py' -Value $smk
& $py -m dev._smoke | Out-Null
...
if($LaunchChat){
  ...
  Start-Process $py2 -ArgumentList '-m','dev.brain_chat_shell'
}

$ts=Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
Set-Content -Encoding Ascii -LiteralPath 'reports\FINAL_READY.txt' -Value "[$ts] READY: config ok, GPT-5 locked, deps ok, background ok, smoke ok, chat launched=$($LaunchChat.IsPresent)"
