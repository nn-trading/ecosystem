...
# --- brain-only autonomous chat (no planner noise) ---
$chat=@'
...
def chat_llm(messages):
    ...
    key = get_key()
    if not key: return "(no OPENAI_API_KEY/api_key.txt)"
...
def main():
    ...
    while True:
        ...
        if q.lower().startswith('/model'): print(f'(LOCKED to {MODEL})'); continue
        append('user', q)
...
# --- hard stop any background to eliminate spam/noise ---
try { powershell -NoProfile -File '.\start.ps1' -Stop 1 | Out-Null } catch {}

# --- quick smoke (no planner) ---
$smk=@'
...
# --- launch clean chat (no background) ---
...
Set-Content -Encoding Ascii -LiteralPath 'reports\FINAL_READY.txt' -Value "[$ts] BRAIN-ONLY: GPT-5 locked, deps ok, smoke ok, chat launched"
