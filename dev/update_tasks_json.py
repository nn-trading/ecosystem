...
# Auto-detect triage counts and latest snapshot for session updates
repo = Path(__file__).resolve().parent.parent
runs_cur = repo / 'runs' / 'current'

def _load_json_list(p: Path):
    try:
        data2 = json.loads(p.read_text(encoding='ascii', errors='ignore') or '[]')
...
counts = {}
for key, fname in {
    'error': 'eventlog_error.json',
    'exception': 'eventlog_exception.json',
    'fatal': 'eventlog_fatal.json',
    'Traceback': 'eventlog_traceback.json',
}.items():
    arr = _load_json_list(runs_cur / fname)
    counts[key] = len(arr)

# latest run snapshot (exclude 'current')
runs_dir = repo / 'runs'
latest = ''
if runs_dir.exists():
    try:
        latest = max((d for d in runs_dir.iterdir() if d.is_dir() and d.name != 'current'), key=lambda d: d.name).name
    except Exception:
        latest = ''
...
triage_note = f"error={counts.get('error', 0)}; exception={counts.get('exception', 0)}; fatal={counts.get('fatal', 0)}; Traceback={counts.get('Traceback', 0)}"
if counts.get('fatal', 0) == 0 and counts.get('Traceback', 0) == 0:
    triage_note += '; no fatal/traceback found'

upsert(ses, {"id":"TRIAGE-LOGS", "title":"Re-triage recent events for 'error','exception','fatal','Traceback'", "status":"done", "notes":f"{triage_note}; done {now}"})
if latest:
    upsert(ses, {"id":"REFRESH-STATUS", "title":"Update STATUS.md to reflect latest snapshot and states", "status":"done", "notes":f"STATUS.md refreshed to runs/{latest}; HEAD {head}; done {now}"})
