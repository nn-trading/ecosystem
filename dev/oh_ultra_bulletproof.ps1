...
New-Item -ItemType Directory -Force -Path .\logs, .\runs, .\reports, .\reports\chat, .\config, .\var | Out-Null

# 0) Quiet pre-stop and kill any sticky python loops to avoid DB locks
try { powershell -NoProfile -File .\start.ps1 -Stop 1 | Out-Null } catch {}
# targeted kill for known patterns that can keep var\events.db open
Get-CimInstance Win32_Process | Where-Object {
  $_.Name -match 'python.exe' -and (
    $_.CommandLine -match 'from dev import jobs_queue' -or
    $_.CommandLine -match 'dev\\core02' -or
    $_.CommandLine -match 'dev\\eco_cli' -or
    $_.CommandLine -match 'agent.py' -or
    $_.CommandLine -match 'ecosys'
  )
} | ForEach-Object {
...
# 1) ASCII-safe configs (Comms -> Brain; echo off; tail path)
...
# 2) Brain Chat shell (instant LLM reply + planner kick + tail poll; ASCII logs)
$brain=@'
...
def poll_for_reply(timeout=18, min_wait=1.0):
    t0=time.time()
...
def try_model(q):
    ...
    try:
        ...
        sys_prompt = "You are the Ecosystem Brain on Windows. Answer the user directly and briefly. Never ask them for goals; act helpful."
...
def main():
    ...
    while True:
        ...
        append("user", q)
        try_planner(q)
        ans = try_model(q)            # instant reply path
        append("assistant", ans); print(ans)
        extra = poll_for_reply(timeout=14, min_wait=1.0)
...
# 3) One-click runner: stop -> start (quiet) -> launch chat; suppress dashboard noise
$runner=@'
param()
$ErrorActionPreference="Stop"
$env:PYTHONUTF8="1"; $env:PYTHONIOENCODING="utf-8"; $env:ECOSYS_SKIP_DASHBOARD="1"
try { powershell -NoProfile -File .\start.ps1 -Stop 1 | Out-Null } catch {}
# start quietly (hide non-fatal dashboard messages)
...
# 4) Route Comms -> Brain (echo off), apply plan, start background quietly
...
# 5) Smoke chat (non-interactive) + tail probe
$smk=@'
...
$ok_chat = ($out -match "Brain chat ready" -or $out -match "You>" -or $out -match "hello")
...
# 8) Final summary + bundle
$sum=@()
$sum+="ULTRA BULLETPROOF READY"
...
$txt = ($sum -join "`r`n")
Write-Ascii 'reports\ULTRA_BULLETPROOF_READY.txt' $txt
$bundle="runs\oh_ultra_bulletproof_"+(Get-Date -Format 'yyyyMMdd_HHmmss'); New-Item -ItemType Directory -Force -Path $bundle|Out-Null
foreach($f in @('reports\ULTRA_BULLETPROOF_READY.txt','reports\BULLETPROOF_READY.txt','reports\GRAND_ASSERT.txt','reports\GRAND_TEST_REPORT.json','reports\GRAND_TEST_SUMMARY.txt','reports\chat\exact_tail.jsonl')){
